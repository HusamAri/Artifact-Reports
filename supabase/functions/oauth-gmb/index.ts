// oauth-gmb — Google Business Profile (formerly Google My Business)
// OAuth 2.0. Uses the same Google auth surface as oauth-youtube but
// asks for the business.manage scope and stores the first account on
// social_accounts. Location-level selection lands in a follow-up.
//
// POST /start  { workspace_id } → { auth_url }
// GET  /callback?code=&state=  → HTML landing.
...
const CLIENT_ID = Deno.env.get("GMB_CLIENT_ID")!;
const CLIENT_SECRET = Deno.env.get("GMB_CLIENT_SECRET")!;
const REDIRECT_URI = Deno.env.get("GMB_OAUTH_REDIRECT_URI")!;
...
async function handleCallback(url: URL): Promise<Response> {
  ...
  // Fetch the first GMB account for display.
  const acctRes = await fetch(
    "https://mybusinessaccountmanagement.googleapis.com/v1/accounts",
    { headers: { Authorization: `Bearer ${tokens.access_token}` } },
  );
  if (!acctRes.ok) {
    return htmlError(`/accounts failed: ${await acctRes.text()}`);
  }
  ...
  const account = acctJson.accounts?.[0];
  if (!account) return htmlError("No GMB account on this Google identity.");
  const externalId = account.name; // e.g. "accounts/123456"
  ...
  const { error: upsertErr } = await service.from("social_accounts").upsert({
    ...
    display_name: account.accountName ?? "Google Business",
    handle: account.type ?? null,
    avatar_url: null,
  ...
  await service.from("oauth_states").delete().eq("state", state);
  return htmlSuccess(account.accountName ?? "Google Business");
}
